...
# ------------------ Helper: Sum of PR > 99th percentile ------------------ #
def very_heavy_precip_total(precip):
    pr99 = precip.quantile(0.99, dim='time')
    very_heavy_precip = precip.where(precip > pr99, 0.0)
    return very_heavy_precip.sum(dim='time')
# ------------------ Find files ------------------ #
...
for file in all_hist_files:
    try:
        ...
        # Resample to annual and calculate total very heavy precipitation
        pr_very_heavy_annual = pr.resample(time='YE').map(very_heavy_precip_total)

        # Mean across years
        pr_very_heavy_mean = pr_very_heavy_annual.mean(dim='time')

        # Apply region mask
        region_mask_da = region_mask.mask(pr_very_heavy_mean)

        # Aggregate by region
        regional_very_heavy_pr = pr_very_heavy_mean.groupby(region_mask_da).mean()

        # Append to list
        bioregion_veryheavypr_data.append(regional_very_heavy_pr)
...
ensemble_stack = xr.concat(bioregion_veryheavypr_data, dim='model')
...
df = pd.DataFrame({
    ...
    "VeryHeavyRainTotal_mm_per_year": ensemble_mean_by_region.values
}).sort_values(by="VeryHeavyRainTotal_mm_per_year", ascending=False)

print("\n📊 Total Annual PR from Very Heavy Rain (PR > 99th percentile, mm/year):")
print(df)
...
vmin = 0
vmax = bioregions_merged["VeryHeavyRainTotal_mm_per_year"].max()
step = 50
ticks = np.arange(vmin, vmax + step, step)

bioregions_merged.plot(
    column="VeryHeavyRainTotal_mm_per_year",
    cmap="Blues",
    linewidth=0.8,
    edgecolor="black",
    legend=True,
    legend_kwds={
        "label": "Annual Total PR > 99th percentile (mm/year)",
...
ax.set_title("CMIP6: Total Annual PR from Very Heavy Rain Days PR > 99th % (1950–2014)", fontsize=14)
ax.set_axis_off()
plt.tight_layout()
plt.show()