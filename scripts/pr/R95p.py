...
def compute_annual_sum_above_95th(pr, pr95):
...
for model in tqdm(model_dirs, desc="🔁 Processing models"):
    ...
    try:
        ...
        heavy_rain_sum = compute_annual_sum_above_95th(pr, pr95)
...
if model_sum_national:
    ...
    ensemble_sum.name = "AnnualPR_Above95th"
    ensemble_sum.attrs.update({
        "description": "Total Annual Precipitation from PR > 95th Percentile",
        "units": "mm"
    })
    ensemble_sum.to_netcdf(os.path.join(input_dir, "ensemble_heavy_rain_95th_SA.nc"))
...
if model_sum_bioregion:
    ...
    bioregions["HeavyRain95"] = ensemble_bio_sum.values

    fig, ax = plt.subplots(figsize=(10, 8))
    vmin = int(bioregions["HeavyRain95"].min())
    vmax = int(bioregions["HeavyRain95"].max())
    ...
    bioregions.plot(
        ...
        edgecolor="black",
        legend=True,
        legend_kwds={"label": "Annual Rainfall from PR > 95th percentile (mm)", "orientation": "vertical", "ticks": ticks},
        ax=ax
    )

    ax.set_title("CMIP6: Total Annual PR from Heavy Rain (PR > 95th Percentile) by Bioregion (1950–2014)", fontsize=14)
    ax.set_axis_off()
    plt.tight_layout()
    plt.show()
