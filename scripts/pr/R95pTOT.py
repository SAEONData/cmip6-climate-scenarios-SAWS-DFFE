...
def compute_fraction_above_95p(pr, pr95p):
    wet = pr.where(pr >= 1.0)
...
for model in tqdm(model_dirs, desc="🔁 Processing models"):
    ...
    try:
        ...
        pr = xr.concat([ds["pr"] for ds in ds_list], dim="time") * 86400  # convert from kg/m²/s to mm/day
        pr = pr.sel(**sa_bounds)
        ...
        annual_frac = compute_fraction_above_95p(pr, pr95p)
...
if model_frac_national:
    ...
    ensemble_frac.name = "FracWetPR95p"
    ensemble_frac.attrs.update({
        "description": "Fraction of Wet-Day Rainfall from Days with PR > 95th percentile",
        "units": "fraction"
    })
    ensemble_frac.to_netcdf(os.path.join(input_dir, "ensemble_frac_wet_pr95p_SA.nc"))
    print("✅ Saved national ensemble PR fraction > 95th percentile.")
else:
...
if model_frac_bioregion:
    ...
    bioregions["FracWetPR95p"] = ensemble_bio_frac.values

    fig, ax = plt.subplots(figsize=(10, 8))
    vmin = 0
    vmax = round(bioregions["FracWetPR95p"].max(), 2)
    ...
    bioregions.plot(
        ...
        edgecolor="black",
        legend=True,
        legend_kwds={"label": "Fraction PR > 95th on Wet Days", "orientation": "vertical", "ticks": ticks},
        ax=ax
    )

    ax.set_title("CMIP6: Fraction of Wet-Day PR from >95th Percentile by Bioregion (1950–2014)", fontsize=14)
    ax.set_axis_off()
    plt.tight_layout()
    plt.show()
