...
# ------------------ Helper Functions ------------------ #
def compute_pr95(pr, base_period):
    base = pr.sel(time=base_period)
    if base.time.size == 0:
        raise ValueError("⚠️ Base period is empty.")
    return base.groupby("time.dayofyear").reduce(np.percentile, q=95, dim="time")

def compute_fraction_exceeding_95th(pr, pr95):
    doy = pr["time"].dt.dayofyear
    threshold = pr95.sel(dayofyear=doy)
...
for model in tqdm(model_dirs, desc="🔁 Processing models"):
    ...
    try:
        ...
        pr95 = compute_pr95(pr, base_period)
        frac_95 = compute_fraction_exceeding_95th(pr, pr95)

        # National average
        model_pct_national.append(frac_95.mean(dim=["lat", "lon"]))

        # Bioregional average
        region_ids = region_mask.mask(pr)
        bio_means = [frac_95.where(region_ids == r).mean(dim=["lat", "lon"], skipna=True)
...
if model_pct_national:
    ...
    ensemble_pct.name = "WetDayPR_Above95thPct"
    ensemble_pct.attrs.update({
        "description": "Fraction of Wet-Day Rainfall from PR > 95th percentile",
        "units": "%"
    })
    ensemble_pct.to_netcdf(os.path.join(input_dir, "ensemble_wetday_95th_pct_SA.nc"))
...
if model_pct_bioregion:
    ...
    bioregions["Wet95Pct"] = ensemble_bio_pct.values

    fig, ax = plt.subplots(figsize=(10, 8))
    vmin = int(bioregions["Wet95Pct"].min())
    vmax = int(bioregions["Wet95Pct"].max())
    ...
    bioregions.plot(
        ...
        edgecolor="black",
        legend=True,
        legend_kwds={"label": "% of Rainfall (PR > 95th percentile)", "orientation": "vertical", "ticks": ticks},
        ax=ax
    )

    ax.set_title("CMIP6: Fraction of Wet-Day Rainfall from PR > 95th Percentile by Bioregion (1950–2014)", fontsize=14)
    ax.set_axis_off()
    plt.tight_layout()
    plt.show()