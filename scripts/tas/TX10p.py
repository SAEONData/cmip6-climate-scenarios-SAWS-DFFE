...
# ------------------ Helper Functions ------------------ #
def compute_tx10p(tasmax, base_period):
    ...
    base.coords["doy"] = doy
    tx10p = base.groupby("doy").reduce(np.percentile, q=10, dim="time")
    return tx10p.rename({"doy": "dayofyear"})

def compute_cool_day_percentage(tasmax, tx10p):
    ...
    threshold = tx10p.sel(dayofyear=doy)
    cool_days = tasmax < threshold
    ...
    cool_days.coords["year"] = years

    annual_pct = 100 * cool_days.groupby("year").mean(dim="time")
    return annual_pct
...
for model in tqdm(model_dirs, desc="🔁 Processing models"):
    ...
    try:
        ...
        tx10p = compute_tx10p(tasmax, base_period)
        annual_pct = compute_cool_day_percentage(tasmax, tx10p)
...
if model_pct_national:
    ...
    ensemble_pct.name = "CoolDaysPct"
    ensemble_pct.attrs.update({
        "description": "Percentage of Cool Days (TX < 10th percentile)",
        "units": "%"
    })
    ensemble_pct.to_netcdf(os.path.join(input_dir, "ensemble_cool_days_pct_SA.nc"))
    print("✅ Saved national ensemble cool day percentage.")
else:
...
if model_pct_bioregion:
    ...
    bioregions["CoolDaysPct"] = ensemble_bio_pct.values

    fig, ax = plt.subplots(figsize=(10, 8))
    vmin = int(bioregions["CoolDaysPct"].min())
    vmax = int(bioregions["CoolDaysPct"].max())
    ...
    bioregions.plot(
        ...
        edgecolor="black",
        legend=True,
        legend_kwds={"label": "% of Cool Days", "orientation": "vertical", "ticks": ticks},
        ax=ax
    )

    ax.set_title("CMIP6: % of Days with TX < 10th Percentile by Bioregion (1950–2014)", fontsize=14)
    ax.set_axis_off()
    plt.tight_layout()
    plt.show()
